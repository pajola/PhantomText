    ...
    def __init__(self, modality="default", file_format="pdf"):
        if file_format == "pdf" or file_format == "html":
            ...
            # Modality not valid, raise error
                ...
                raise ValueError(
                    f"Modality {modality} is not valid. Valid modality is 'default' (font-size = 0 ) or 'close-to-zero' (font-size=0.1)"
                )
            ...
            if modality == "default":
                ...
                raise ValueError(
                    f"Modality {modality} is not valid. Valid modality is 'default' for docx (font-size = 1, minimum possible )"
                )
        else:
            raise ValueError(
